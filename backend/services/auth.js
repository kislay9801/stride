...
async function provisionUser(decoded) {
  const existing = await get('SELECT id FROM users WHERE firebase_uid = ?', [decoded.uid]);
  if (existing) return existing.id;

  const name = decoded.name || (decoded.email ? decoded.email.split('@')[0] : 'Student');
  const id = (
    await run(
      'INSERT INTO users (name, email, firebase_uid, avatar_url, cognitive_load, ai_insight) VALUES (?, ?, ?, ?, 50, NULL)',
      [name, decoded.email || null, decoded.uid, decoded.picture || null]
    )
  ).lastInsertRowid;

  // Seed only an empty week scaffold so the chart axis renders — no fake hours.
  for (const [w, i] of [['M', 0], ['T', 1], ['W', 2], ['T', 3], ['F', 4], ['S', 5], ['S', 6]]) {
    await run('INSERT INTO daily_focus (user_id, weekday, day_index, hours) VALUES (?, ?, ?, 0)', [id, w, i]);
  }
  return id;
}
...
export async function authMiddleware(req, res, next) {
  ...
  try {
    ...
    req.userId = await provisionUser(decoded);
